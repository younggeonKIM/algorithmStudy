...
//4299
public class BronzIVTest03 {

//	문제
//	원섭이는 잉글랜드 4부리그 풋볼 리그 2에서 활약하는 AFC 윔블던을 좋아한다. 이 팀은 2002년 윔블던 FC가 밀턴 킨스로 연고 이전을 감행하자 윔블던의 서포터들이 스스로 나서 창단한 팀이다. 
//	윔블던 FC는 1988년에 당시 최고의 팀인 리버풀을 꺾고 FA컵을 우승하면서 엄청난 활약을 했었다. 하지만 1989년 4월 15일 축구역사상 최대 비극적인 사건 중 하나인 힐스보로 참사가 발생하였고, 
//	1990년 1월 테일러 리포트가 발표되었다. 
...
//	하지만, FA Cup은 하부 리그 팀은 낮은 라운드부터 대회에 참가하고, 
...
//	첫째 줄에 두 팀의 경기 결과를 출력한다. 득점을 많이 한 쪽을 먼저 출력한다. 만약, 그러한 합과 차를 갖는 경기 결과가 없다면, -1을 출력한다.
	...
	public static void main(String[] args) {
		...
		// get integer sum and integer difference
		int sum = s.nextInt();
		int differ = s.nextInt();
		// the bigger integer is parameter a, smaller one is parameter b in Object pair
		
		int a =0 ; int b =0;
		boolean existsPlayRes = false;
		// use loop statement for getting pairs of two integers to sum up the integer sum parameter value
		for(int i = sum; i>= sum/2; i--) {
			a = i; b= sum-i;
			// find the pairs that two elements' subtracted value fits the integer differ parameter value
			if(a-b == differ) {
				existsPlayRes = true;
				break;
			}
		}
		// print the pairs or -1
		if(existsPlayRes) {
